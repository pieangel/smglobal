...
procedure MoveRow( aGrid : TStringGrid; FromIndex, ToIndex: integer);
...
begin
  ...
  try
    {if aForm <> nil then
    begin
      aDlg.Position := poScreenCenter;//poOwnerFormCenter;
    end else
    }
      aDlg.Position := poScreenCenter;
...
begin
  if iSide = 0 then
    Result := ''
  else Result := IfThenStr( iSide > 0, '매수','매도');
end;
...
procedure MoveRow( aGrid : TStringGrid; FromIndex, ToIndex: integer);
begin
  TmpGrid( aGrid ).MoveRow( FromIndex, ToIndex);
end;
...
var
  ...
  s : string;
begin
  // 소수점 최대 10자리 이상 수가 있을까?
  j := 0;
  s := Format('%.10f', [ Value ] );
  iLen  := Length( s );
  // 뒤에서부터 0 이 아닌 자리수를 찾는다.
...
function IsEqual( x, y : double ) : boolean;
begin
  Result  := ((x - EPSILON2 ) <= y ) and ( y <= (x + EPSILON2));
end;
