...
type
  ...
  public
    ...
    property SideToStr : string read GetSideToStr;
    ...
    property Liquidated : boolean read FLiquidated write FLiquidated;
    ...
    property IsRecover : boolean read FIsRecover write FIsRecover;
      // order & fill info
    ...
    property AbleQty  : integer read FAbleQty write FAbleQty;
  ...
  public
    ...
    procedure CheckHavePosition( aAccount : TAccount );
...
begin
  ...
  FBidParticipation := aPosition.FBidParticipation;
  FAskParticipation   := aPosition.FAskParticipation;
    // quote
...
begin
  ...
  begin
    // 청산 주문후 잔고 0 이 되는 시점..
    if FLiquidated then FLiquidated := false;
      // P&L
    if FSymbol.Spec <> nil then
    begin
      FEntryPL := FEntryPL
...
begin
  ...
  if FQuote <> nil then
  begin
    if FQuote.Bids.VolumeTotal * 100 = 0 then
      FBidParticipation := 0
    else
      FBidParticipation := FActiveBuyOrderVolume / FQuote.Bids.VolumeTotal  * 100;

    if FQuote.Asks.VolumeTotal * 100 = 0 then
      FAskParticipation := 0
    else
      FAskParticipation := FActiveSellOrderVolume / FQuote.Asks.VolumeTotal  * 100;
  end
  else begin
    FBidParticipation := 0;
    FAskParticipation := 0;
...
begin
  Result := Format('(%s,%s,%d,%s)',
                    [FAccount.Code, FSymbol.Code,
                     FVolume, FSymbol.PriceToStr( FAvgPrice )]);
...
function TPosition.GetSideToStr: string;
begin
  if Volume > 0 then
    Result := '매수'
  else if Volume < 0 then
...
begin
  ...
  if aInvestPosition.ReqAbleQty then
  begin
    aPosition.ReqAbleQty  := aInvestPosition.ReqAbleQty;
    aPosition.AbleQty     := aInvestPosition.AbleQty;
    gEnv.Engine.TradeBroker.PositionEvent( aPosition );
  end
  else begin
    aInvestPosition.ReqAbleQty := true;
    gEnv.Engine.SendBroker.ReqAbleQty(aInvest,  aInvestPosition.Symbol );
  end;
  
end;

procedure TPositions.Reset;
begin
  Clear;
end;
...
procedure TPositions.CheckHavePosition(aAccount: TAccount);
var
  I: Integer;
  aPos : TPosition;
  aFund: TFund;
...
begin
  ...
  begin
    aPos  := Positions[i];
    if ( aPos <> nil ) and ( aPos.FAccount = aAccount ) then
    begin
      if ( aPos.Volume <> 0 ) and  ( not aPos.IsRecover ) then begin
        gEnv.EnvLog( WIN_TEST, Format('Position Force Zero : %s', [ aPos.Represent])  );
        aPos.SetPosition( 0, 0, 0 );
        aPos.CaclOpePL( aPos.Symbol.Last);
        //
        if not ( aAccount is TInvestor ) then begin
          gEnv.Engine.TradeBroker.PositionEvent( aPos, 110);// POSITION_UPDATE);

          aFund := gEnv.Engine.TradeCore.Funds.Find( aAccount );
          if aFund <> nil then
          begin
            aFundPos  := gEnv.Engine.TradeCore.FundPositions.Find( aFund, aPos.Symbol  );
            if aFundPos <> nil then
              if aFundPos.Positions.FindPosition( aPos ) then begin
                aFundPos.RecoveryPos( aPos.Volume, aPos.AvgPrice);
                gEnv.Engine.TradeBroker.PositionEvent( aFundPos, 231 );//FPOSITION_UPDATE );
              end;
          end;
        end;  //  서브 계좌일때만
      end;

      aPos.IsRecover := false;
    end;
...
begin
  ...
  dFixed  := dFixed + aAccount.FixedPL[dtUSD];
end;
...
procedure TFundPosition.UpdatePosition;
var
  iNewQty, iVol, I: Integer;
  aPos : TPosition;
  dAmt, dAvg, dPL, dOte : double;

begin

  iVol := 0;
  dPL  := 0;  dOte := 0;  dAvg := 0;   dAmt := 0;

  for I := 0 to Positions.Count - 1 do
  begin
    aPos  := Positions.Positions[i];
    dPL   := aPos.EntryPL + dPL;
    dOte  := aPos.EntryOTE  + dOte;
    dAmt  := aPos.TradeAmt  + dAmt;

    if aPos.Volume = 0 then Continue;

    iNewQty := iVol + aPos.Volume;

    if iVol = 0 then
      dAvg := aPos.AvgPrice
    else begin
      // 서로 다른 방향일때
      if iVol * iNewQty  < 0 then
      begin
        dAvg := aPos.AvgPrice;
      end else
      if iNewQty = 0 then
      ...
      if Abs(iNewQty) > Abs(iVol)  then
      begin
        dAvg := (dAvg * iVol + aPos.AvgPrice * aPos.Volume) / iNewQty;
      end;
    end;
...
begin
  ...
  begin
    dFixed  := dFixed + aFund.FundAccount[j].FixedPL[dtUSD];
...
procedure TFundPositions.Reset;
begin
  Clear;
end;
