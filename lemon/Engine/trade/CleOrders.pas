...
//    FVirAcptSpan: integer;
    ...
    procedure SetHanaOrderNo(const Value: string);
    function GetTypeToStr: string;
  ...
  public
    ...
    procedure SetQty( iOrd , iAct, iFill, iMod, iCnl : integer );
    ...
    property TypeToStr : string read GetTypeToStr;
    ...
    property GroupNo: string read FGroupNo write FGroupNo;
      // only 하나증권 주문번호가 열라 길다
    property HanaOrderNo : string read FHanaOrderNo write SetHanaOrderNo;
  ...
  public
    ...
    procedure CheckActiveOrder( aInvest : TInvestor );
...
procedure TOrder.SetHanaOrderNo(const Value: string);
begin
  FHanaOrderNo := Value;
end;
...
procedure TOrder.SetQty(iOrd, iAct, iFill, iMod, iCnl: integer);
begin
{
  gEnv.EnvLog( WIN_TESt, Format('Qty Set : %s, %s, %s, %s, %d (%d,%d)(%d,%d)(%d,%d)(%d,%d)(%d,%d)',
    [ FAccount.Code, FSymbol.Code, SideToStr, FSymbol.PriceToStr( Price ), OrderNo,
      FOrderQty, iOrd, FActiveQty, iAct, FFilledQty, iFill,
      FCanceledQty , iCnl,  FConfirmedQty, iMod ]));
...
begin
  if FState in [ osSrvAcpt {osSrvAcpt}] then
  begin
    //FResults.Add( aResult );
    FState  := osActive;
    FModify := false;
...
function TOrder.GetTypeToStr: string;
begin

end;

function TOrder.GetSideToStr: string;
begin
  Result := '';
  case FSide of
    1 : Result := '매수';
...
begin
  Result := Format('(%s,%s', [ FAccount.Code, FSymbol.Code])
            + Format(',%s,%s,%s', [OrderTypeDesc, TimeToMarketDesc, PriceControlDesc])
...
begin
  Result :=  Format('(%s, %s ', [ FAccount.Code, FSymbol.Code]) + GetOrderName +
...
begin
  ...
  //gEnv.EnvLog( WIN_TEST, Format(' :%s,%s,%d,%d', [stInvestCode,stPrice, iSide, iQty])   );
  ...
  begin
    ...
    begin
      ...
      //gEnv.EnvLog( WIN_TEST, Format('%d:%s,%s,%d,%d', [i, aOrder.Account.InvestCode,stCmpPrice, aOrder.Side, aOrder.OrderQty])   );
...
procedure TOrders.CheckActiveOrder(aInvest: TInvestor);
var
  I: Integer;
  aOrder : TOrder;
begin
  for I := FActiveOrders.Count - 1 downto 0 do
  begin
    aOrder  := FActiveOrders.Orders[i];
    if  aOrder.Account.InvestCode = aInvest.Code then
    begin
      if  not aOrder.IsRecover  then
      begin
        gEnv.EnvLog( WIN_TEST, Format('Order Force Closed : %s', [ aOrder.Represent3])  );
        aOrder.Confirm( aOrder.ActiveQty, now);
        gEnv.Engine.TradeBroker.UpdateOrd( aOrder, 103 );//ORDER_CONFIRMED );
      end;
      aOrder.IsRecover := false;
    end;
  end;
