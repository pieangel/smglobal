...
type
  ...
  public
    ...
    function GetHitPrice( iSide, iTick : integer ) : double; overload;
    function GetHitPrice( iSide, iTick : integer; dPrice : double ) : double; overload;
  end;
...
function TQuote.GetHitPrice(iSide, iTick: integer; dPrice: double): double;
begin
  Result := 0;

  if ( dPrice < EPSILON ) then
    Exit;
  Result := TicksFromPrice( FSymbol, dPrice, iTick * iSide )
end;
...
begin
  ...
  if FSymbol.TabletHigh = 0 then
    FSymbol.TabletHigh := TicksFromPrice(FSymbol, FSymbol.Last,  1200);
  if FSymbol.TabletLow = 0 then
    FSymbol.TabletLow  := Max(TicksFromPrice(FSymbol, FSymbol.Last, - 1200),
          FSymbol.Spec.TickSize );
    // notify to the QuoteBroker
  if Assigned(FOnUpdate) then
    FOnUpdate(Self);
{
  if FSymbol.Code = '6BZ16' then
    if assigned( gEnv.OnLog ) then
      gEnv.OnLog(self, '');
}
    // notify
  FDistributor.Distribute(Self, 0, Self, 0, 1);

  //if FDistributor.Count <= 0 then
  //  gEnv.EnvLog( WIN_TEST, Format('%s yet', [ FSymbol.Code]) );
...
begin
  ...
  begin
    ...
    bSub := true;
    if ( bSub ) and (Assigned(FOnSubscribe)) then
      FOnSubscribe(Result);
  end;
  {
  if bSub then
    gEnv.EnvLog( WIN_TEST, stCode + ' : add sub' );
  }
  ...
  //gEnv.EnvLog( WIN_TEST, Format('%s Subscribe', [ Result.Symbol.Code]) );
