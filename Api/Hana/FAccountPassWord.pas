...
type
  ...
  private
    ...
    function FindPassword(aInvest: TInvestor): string;
  public
    { Public declarations }
    Datas : TList;
...
procedure TFrmAccountPassWord.FormDestroy(Sender: TObject);
begin
  Datas.Free;
end;
...
function TFrmAccountPassWord.FindPassword( aInvest : TInvestor ) : string;
var
  I: Integer;
  aItem : TTmpAcntData;
begin
  ...
  begin
    aItem := TTmpAcntData( Datas.Items[i] );
    if aITem.Code = aInvest.Code then
    begin
      Result := aItem.Pass;
      break;
    end;
...
begin
  ...
  stFileName  := '\'+ DIR_DATA + '\' +
    Format('%s_%s_%s', [gEnv.ConConfig.UserID,
            ifThenStr(gEnv.ConConfig.RealMode, 'Real','Mock'), FILE_ACNT_INFO ])  ;
...
begin
  ...
  try
    stCode := trim( aEnvFile.Lines[iP] ) ;
    Inc(iP);          //--1. 계좌
    stPass := trim( aEnvFile.Lines[iP] );
    Inc(iP);          //--3. 비번
    Result := True;
