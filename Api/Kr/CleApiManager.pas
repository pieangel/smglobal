...
const
  MAX_CNT  : array [0..1] of integer = ( 30, 7 );
  MAX_CNT2 = 2;
  QRY_CNT = 2;
...
type
  ...
  public
    ...
    procedure TimeProc3( Sender : TObject );
    ...
    property AcntQryList : TList read FAcntQryList;
...
begin
  ...
  FAcntTimer  := TTimer.Create( nil );
  FAcntTimer.Interval := 100;
  FAcntTimer.Enabled  := false;
  FAcntTimer.OnTimer  := TimeProc3;
...
begin
  ...
  try
    ...
    begin
      ...
      if idx >= 0 then
        if FQryCount[idx] > MAX_CNT[idx] then
          break;

      iRes := RequestData( aItem.TrCode, aItem.Data, aitem.Size );
      if iRes <> 0 then
        break;
...
procedure TApiManager.TimeProc3(Sender: TObject);
var
  aItem : TSrvSendItem;
  I, iCnt, iRes, idx : Integer;
begin
  ...
  try
      i := FAcntQryList.Count -1 ;
      if i < 0 then Exit;
      
      aItem := TSrvSendItem(  FAcntQryList.Items[i] );
