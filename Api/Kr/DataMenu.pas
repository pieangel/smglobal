...
uses
  ...
  FSingleOrderBoard, FSingleFundOrderBoard,
  FA50Trend , FFundOrderBoard, FleFundMiniPositionList, FA_P2 {, FRiskManage}
  ;
...
begin
  ...
  case (Sender as TComponent).Tag of
    ...
    306: gEnv.Engine.FormBroker.Open(ID_SINGLE_ORDERBOARD, 0); // order board
    ...
    405: gEnv.Engine.FormBroker.Open(ID_SINGLE_FUND_ORDERBOARD, 0);
...
begin
  ...
  case iFormID of
    ...
    ID_SINGLE_ORDERBOARD  : aForm := TSingleOrderBoardForm.Create(OrpMainForm);
    ...
    ID_A50_TREDN:
        aForm := TFrmA50Trend.Create( OrpMainForm);
    ID_A50_P2:
        aForm := TFrmA_P2.Create( OrpMainForm);
                 {
    ID_VOLTRADE:
        aForm := TFrmRiskManage.Create( OrpMainForm );
                  }
    ID_FUND_ORDERBOARD:
        aForm := TFundBoardForm.Create( OrpMainform );

    ID_SINGLE_FUND_ORDERBOARD:
        aForm := TSingleFundOrderBoardForm.Create( OrpMainForm );
...
begin
  ...
  case iFormID of
    ...
    ID_SINGLE_ORDERBOARD:
      if aForm is TSingleOrderBoardForm then
        (aForm as TSingleOrderBoardForm).LoadEnv(aStorage);
    ...
    ID_A50_TREDN:
        if aForm is TFrmA50Trend  then
        ( aForm as TFrmA50Trend).LoadEnv( aStorage );

    ID_A50_P2:
        if aForm is TFrmA_P2  then
        ( aForm as TFrmA_P2).LoadEnv( aStorage );
        {
    ID_VOLTRADE:
        if aForm is TFrmRiskManage  then
        ( aForm as TFrmRiskManage).LoadEnv( aStorage );
          }
    ID_FUND_ORDERBOARD:
    ...
    ID_SINGLE_FUND_ORDERBOARD:
        if aForm is TSingleFundOrderBoardForm  then
        ( aForm as TSingleFundOrderBoardForm).LoadEnv( aStorage );
...
begin
  ...
  case iFormID of
    ...
    ID_SINGLE_ORDERBOARD:
      if aForm is TSingleOrderBoardForm then
        (aForm as TSingleOrderBoardForm).SaveEnv(aStorage);
    ...
    ID_A50_TREDN:
        if aForm is TFrmA50Trend  then
        ( aForm as TFrmA50Trend).SaveEnv( aStorage );

    ID_A50_P2:
        if aForm is TFrmA_P2  then
        ( aForm as TFrmA_P2).SaveEnv( aStorage );
         {
    ID_VOLTRADE:
        if aForm is TFrmRiskManage  then
        ( aForm as TFrmRiskManage).SaveEnv( aStorage );
          }
    ID_FUND_ORDERBOARD:
        if aForm is TFundBoardForm  then
    ...
    ID_SINGLE_FUND_ORDERBOARD:
        if aForm is TSingleFundOrderBoardForm  then
        ( aForm as TSingleFundOrderBoardForm).SaveEnv( aStorage );
