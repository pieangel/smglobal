...
type
  ...
  private
    ...
    procedure DoAutoStop(aPos: TPosition; iQty: integer; bPrf, bLos: boolean;
      iPrf, iLos, iTick: integer; pcValue: TPriceControl);
...
begin
  ...
  // 가격 구하기
  dAvgPrc := FTablet.GetTickValue( aPos.Symbol.Spec.Market, aPos.AvgPrice );

  if aPos.Volume > 0 then begin
    dLosPrc := TicksFromPrice(aPos.Symbol, dAvgPrc, -iLos); // dAvgPrc  - ( FPosition.Symbol.Spec.TickSize * iLos );
    dPrfPrc := TicksFromPrice(aPos.Symbol, dAvgPrc, iPrf); //dAvgPrc  + ( FPosition.Symbol.Spec.TickSize * iPrf );
    iSide   := -1;
  end
  else if aPos.Volume < 0 then begin
    dPrfPrc := TicksFromPrice(aPos.Symbol, dAvgPrc, -iPrf); //dAvgPrc  - ( FPosition.Symbol.Spec.TickSize * iPrf );
    dLosPrc := TicksFromPrice(aPos.Symbol, dAvgPrc, iLos); // dAvgPrc  + ( FPosition.Symbol.Spec.TickSize * iLos );
    iSide   := 1;
  end else Exit;
