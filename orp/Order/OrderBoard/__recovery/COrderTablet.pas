...
//    FIV : Double ;
  ...
  private
    ...
    procedure SetStopPrice(const Value: double);
  ...
  public
    ...
    procedure SetLine( aImgs : array of TImage32);
    procedure SetLine2(aImg : TImage32);
    ...
    property StopPrice : double read FStopPrice write SetStopPrice;
...
begin
  ...
  if (FFristQuotes[ptShort] < FStartIndex) or (FFristQuotes[ptShort] > FEndIndex) then
    iBottom := -1
  else begin
    iTop :=  FDataTop + (FEndIndex-FFristQuotes[ptShort]) * DEFAULT_HEIGHT ;
    iBottom := iTop + DEFAULT_HEIGHT;
  end;
  FImgLine.Top  := iBottom;

  try

    if (FStopIndex < FStartIndex) or (FStopIndex > FEndIndex) then begin
      for I := 0 to High( FImgStop) do FImgStop[i].Top := -5;
    end
    else begin
      iTop :=  FDataTop + (FEndIndex-FStopIndex) * DEFAULT_HEIGHT ;
      iBottom := iTop + DEFAULT_HEIGHT;

      FImgStop[0].Top := iTop;
      FImgStop[1].Top := iTop;
      FImgStop[2].Top := iBottom;
      FimgStop[3].Top := iTop;
  {
    gEnv.EnvLog( WIN_TEST,  Format('위치 : %d, %d ---> (%d, %d), (%d, %d),(%d, %d),(%d, %d)', [
      iTop, iBottom,
      FImgStop[0].Left, FImgStop[0].Top,
      FImgStop[1].Left, FImgStop[1].Top,
      FImgStop[2].Left, FImgStop[2].Top,
      FImgStop[3].Left, FImgStop[3].Top])
             );
   }
    end;
...
procedure TOrderTablet.SetStopPrice(const Value: double);
begin
  if (FSymbol = nil) or (FSymbol.Spec = nil) then exit ;
  FStopPrice := Value;

  FStopIndex := FindIndex(Value);
  DrawQuoteLine;
end;
...
begin
  ...
  if FCols[tcCount,  FPositionTypes[stLeft]].Visible then
    iLeft  := FCols[tcCount,  FPositionTypes[stLeft]].Left
  else
    iLeft  := FCols[tcQuote,  FPositionTypes[stLeft]].Left;

  if FCols[tcCount, FPositionTypes[stRight]].Visible then
    iRight := FCols[tcCount, FPositionTypes[stRight]].Right
  else
    iRight := FCols[tcQuote, FPositionTypes[stRight]].Right;

  try

  {
      트레일링 스탐..사각형
       ___________0______________________
    3 |                                 |  1
      -----------------------------------
                  2
  }
  if FImgStop[0] <> nil then
  begin

    FImgStop[0].Width := abs(iRight - iLeft);
    FImgStop[0].Left  := iLeft;

    FImgStop[1].Width := T_STOP_WIDTH;
    FImgStop[1].Left  := iRight;

    FImgStop[2].Width  := abs(iRight - iLeft);
    FImgStop[2].Left  := iLeft;

    FImgStop[3].Width  := T_STOP_WIDTH;
    FImgStop[3].Left  := iLeft;
  end;
...
begin
  ...
  begin
    FHighLimit:= FSymbol.TabletHigh;
    FLowLimit := FSymbol.TabletLow;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        Price := dPrice;
{$IFDEF DONGBU_STOCK}
        PriceDesc := FSymbol.PriceCrt.GetString( dPrice);
{$ELSE}
...
begin
  ...
  FImgLine.Width    := FWidth;
  FImgLine.Left     := aPaintBox.Left;
  ...
  begin
    if FCols[tcCount, ptShort].Visible then begin
      Left := FCols[tcCount, ptShort].Left;
      Width := FCols[tcCount, ptShort].Width +  FCols[tcQuote, ptShort].Width -1;
    end else begin
      Left := FCols[tcQuote, ptShort].Left;
      Width := FCols[tcQuote, ptShort].Width -1;
    end;
...
procedure TOrderTablet.SetLine(aImgs : array of TImage32);
var
  I: Integer;
begin
  for I := 0 to High(aImgs) do
  begin
    FImgStop[i]  := aImgs[i];
    if ( i mod 2 ) = 0 then
      FImgStop[i].Height := T_STOP_WIDTH
    else begin
      FImgStop[i].Height := DEFAULT_HEIGHT;
      FImgStop[i].Width  := T_STOP_WIDTH;
    end;
  end;
end;

procedure TOrderTablet.SetLine2(aImg: TImage32);
begin
  FImgLine  := aImg;
  FImgLine.Height := 1;
end;
