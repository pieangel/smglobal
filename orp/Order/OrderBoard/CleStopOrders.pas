...
type
  ...
  public
    ...
    //
    property TrdStop : TStopORderItem read FTrdStop write FTrdStop;
...
begin
  ...
  stLog :=
  Format( 'Delete : %s, %s, %s, %.2f, %d, %d, %s, (%s)',
  [
...
begin
  ...
  if aStop.Price > aStop.LastPrice + DOUBLE_EPSILON then
  begin
    // 지금 현재가가 크거나 같으면 주문..ㄱㄱ
    if aQuote.Last > aStop.Price  -DOUBLE_EPSILON then begin
      //stLog := Format('order
  ...
  if aStop.Price + DOUBLE_EPSILON < aStop.LastPrice then
  begin
    // 지금 현재가가 같거나 작을때 주문 ㄱㄱ
    if aStop.Price  > aQuote.Last- DOUBLE_EPSILON then begin
...
begin
  ...
  begin
    ...
    begin
      ...
      aStop.soType := soCancel;
      stLog := Format('Send Stop Order : %s, %s,%s,%s,prc:%s,cur:%s,last:%s, ordPrc:%s, %d, %d', [
          aStop.Account.Code,
          aStop.Symbol.ShortCode,
