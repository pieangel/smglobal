...
uses
  Classes, Graphics, Buttons, Windows,
  CleSymbols
  ;
...
type
  ...
  TEventType = ( etQty, etSpace, etStop, etMain, etInterest );
  ...
  TSectorItem = class( TCollectionItem )
  public
    Code : string;
    Name : string;
  end;

  TSectorItems = class( TCollection )
  public
    Constructor Create;
    function  New( stcode, stname : string ) : TSectorItem ;
  end;
  ...
  TFavorSymbolItem = class( TCollectionItem )
  public
    button  : TSpeedButton;
    Symbol  : TSymbol;
  end;

  TFavorSymbols = class( TCollection )
  public
    Constructor Create;
    function New( btn : TSpeedButton ) : TFavorSymbolItem;
  end;
...
constructor TSectorItems.Create;
begin
  inherited Create( TSectorITem );
end;

function  TSectorItems.New( stcode, stname : string ) : TSectorItem ;
begin
  Result  := add as TSectorITem;
  Result.Code := stCode;
  Result.Name := stname;
end;
...
constructor TFavorSymbols.Create;
begin
  inherited Create( TFavorSymbolItem );
end;

function TFavorSymbols.New(btn: TSpeedButton): TFavorSymbolItem;
begin
  Result  := Add as  TFavorSymbolItem;
  Result.button := btn;
  Result.Symbol := nil;
end;
