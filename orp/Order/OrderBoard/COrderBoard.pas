...
uses
  ...
  ClePositions, CleOrders, COBTypes, CleStopOrders, FmQtySet, FmLiqSet,
    // app: orderboard
  COrderTablet, CTickPainter, CHogaPainter, CleFORMOrderItems,  CleTrailingStops,
  DBoardParams,
...
type
  ...
  private
    ...
    FIsSingle: boolean;
    FOnLogEvent: TBoardLogEvent;

    function GetWidth: Integer;
    procedure SpeedButtonPrefsClick(Sender: TObject);
    procedure StopProfitNLossClick( Sender: TObject);
    procedure TrailingStopClick( Sender : TObject );
    procedure TrailingStopEditChange( Sender : TObject );
    procedure TrailingStopConnChange( Sender : TObject );
    ...
    procedure StopCancel(iSide: integer);
  ...
  public
    ...
    // autostop...
    // Multi
    procedure OnFillExec; overload;
    procedure OnFill( aOrder : TOrder); overload;
    // single
    procedure OnFillExec( bPrf, bLos : boolean; iPrf, iLos, iTick : integer; pcValue : TPriceControl); overload;
    procedure OnFill( aOrder : TOrder;
      bPrf, bLos : boolean; iPrf, iLos, iTick : integer; pcValue : TPriceControl); overload;
    //............................................

    procedure OnTrailingStop( bStart : boolean );
    procedure OnFundTrailingStop( bStart : boolean );
    procedure OnAcntTrailingStop( bStart : boolean );
    procedure OnTrailingStopEvent(Sender: TObject; Value : boolean );
    //procedure OnTTrailingStopMessage( Sender : TObject; iCalc, iMax, iNow : integer );
    ...
    function CheckTrailingStop( aSymbol : TSymbol ) : boolean;
      // objects
    ...
    property OnLogEvent : TBoardLogEvent read FOnLogEvent write FOnLogEvent;
      //
  ...
  public
    constructor Create(aForm: TForm; aPanel: TPanel; bFund : boolean;  bSingle : boolean = false);
...
uses ClePriceItems, MMSystem, GAppEnv, GleConsts, GleLib, CleFills,
  FConfirmLiqMode , GR32,   FSingleORderBoard,  FSingleFundOrderBoard,
  FOrpMain   , Dialogs
  ;
...
begin
  ...
  FIsSingle := TOrderBoards( Coll ).IsSingle;
    // create controls
  CreateControls;
    // object: tablet
...
begin
  ...
  begin
    ...
    cbTrailingStop.OnClick  := TrailingStopClick;
    OnEditChange := TrailingStopEditChange;
    OnConfChange := TrailingStopConnChange;
  end;
      // tablet panel
...
begin
  if (FPosition <> Value) then begin//and ( Value <> nil )then
    FPosition := Value;
    // 주문가능수 요청..
...
begin
  ...
  if (FIsSingle) and  Assigned(OnPosEvent) then
  begin
    if FIsFund then
  ...
  begin
    ...
    Cells[ 5, 1] := Formatfloat('#,##0.###',  dEval / iDiv );
...
begin
  ...
  begin
    if ( FPosition.Symbol <> nil ) and ( FPosition.Symbol.Quote <> nil ) then
      TrailingStop.Observer( FPosition.Symbol.Quote as TQuote, true );
  end;
end;
...
begin
  ...
  begin
    ...
    btnSellAbleQty.Caption := Format('가능(%d)', [ aPosition.AbleQty ]);
    btnSellAbleQty.Tag     := aPosition.AbleQty;
...
procedure TOrderBoard.TrailingStopClick(Sender: TObject);
begin
  with  FLiqset do
  begin
    DoLog( Format('트레일링스탑 체크박스 클릭 %s --> 설정 [ %s, %s, %s, %s(%s) ', [
      ifThenStr( cbTrailingStop.Checked, 'On', 'Off') ,
      edtBaseLCTick.Text ,
      edtPLTick.Text ,
      edtLCTick.Text ,
      edtLiqTick2.Text ,
      ifThenStr( cbLiqType2.ItemIndex = 0 , '시장가','상대호가')])       );
    OnTrailingStop( cbTrailingStop.Checked );

    if cbTrailingStop.Checked then begin
     // lbTrdStopDesc.Caption := '[T Stop On]';
      cbTrailingStop.Color   := clSkyBlue;
       {
...
procedure TOrderBoard.TrailingStopConnChange(Sender: TObject);
var
  iTag : integer;
begin
  ...
  case iTag of
    4,5,6 :
      begin
        if not FTrailingStop.CheckParam then
          with FLiqSet do
            case iTag of
                4 :  edtBaseLCTick.Text := IntToStr( FTrailingStop.Param.BaseTick );
                5 :  edtPLTick.Text     := IntToStr( FTrailingStop.Param.PLTick );
                6 :  edtLCTick.Text     := IntToStr( FTrailingStop.Param.LCTick );
            end;
      end;
    else FTrailingStop.UpdateParam( iTag, FLiqSet.cbLiqType2.ItemIndex ); // 콤보박스..
  end;

end;

procedure TOrderBoard.TrailingStopEditChange(Sender: TObject);
var
  iVal : integer;
  bCalc, bRes : boolean;
begin
  ...
  case (Sender as TComponent).Tag of
    ...
    4,5,6,7 :
      ...
      begin
        ...
        if ( FTrailingStop <> nil ) and ( cbTrailingStop.Checked ) then begin
          FTrailingStop.UpdateParam( (Sender as TComponent).Tag, iVal );
          // 업다운 버튼의 포커스를 죽일수 없어서 포스트메세지 사용 ( 센드 X )
          if bCalc then
            PostMessage( FLiqSet.Handle, WM_LIQSET_MESSAGE, (Sender as TComponent).Tag, iVal );
        end;
...
begin
  ...
  case (Sender as TPanel).Tag of
    SHORT_ST_CANCEL   :
      begin
        stLog := ' 매도 STOP  일괄취소';
        StopCancel( -1 );// if FStopOrder <> nil then FStopOrder.Cancel(-1) ; //= -2;
      end;
    1,0,-1  :
      begin
        stLog := Format(' %d 일괄취소', [ (Sender as TPanel).Tag ]);
        AllCancels( (Sender as TPanel).Tag );
      end;
    LONG_ST_CANCEL    :
      begin
        stLog := ' 매수 STOP  일괄취소';
  ...
  {
  case (Sender as TPanel).Tag of
    SHORT_ST_CANCEL   : StopCancel(-1) ; //= -2;
    1,0,-1  :  AllCancels( (Sender as TPanel).Tag );
    LONG_ST_CANCEL    : StopCancel(1); // = 2;
  end;
  }
end;

procedure TOrderBoard.StopCancel( iSide :integer );
var
  aStop : TStopOrder;
  i : Integer;
begin
  ...
  begin
    ...
    if FStopOrder <> nil then  FStopOrder.Cancel(iSide) ;
end;
...
function TOrderBoard.GetParam : TStopParam;
var
  aColl  : TSingleOrderBoardForm;
  aColl2 : TSingleFundOrderBoardForm;
begin
  if FIsSingle then
  begin
    if FIsFund then
    begin
      aColl2 := ( Collection as TOrderBoards).FOwnerForm as TSingleFundOrderBoardForm;
      Result.BaseTick := StrToInt( aColl2.edtBaseLCTick.Text );
      Result.PLTick   := StrToInt( aColl2.edtPLTick.Text );
      Result.LCTick   := StrToInt( aColl2.edtLCTick.Text );
      Result.IsMarket := aColl2.rbMarket2.Checked;
      Result.OrdTick  := StrToInt( aColl2.edtLiqTick2.Text );
    end else
    begin
      aColl := ( Collection as TOrderBoards).FOwnerForm as TSingleOrderBoardForm;
    ...
    with FLiqSet do
    begin
      Result.BaseTick := StrToInt( edtBaseLCTick.Text );
      Result.PLTick   := StrToInt( edtPLTick.Text );
      Result.LCTick   := StrToInt( edtLCTick.Text );
      Result.IsMarket := cbLiqType2.ItemIndex = 0;
      Result.OrdTick  := StrToInt( edtLiqTick2.Text );
    end;
end;

procedure TOrderBoard.OnFundTrailingStop(bStart: boolean);
begin
  ...
  begin
    ...
    begin
      ...
      if FTrailingStop.CheckParam( GetParam ) then
      begin
...
procedure TOrderBoard.OnAcntTrailingStop(bStart: boolean);
begin
  if ( FAccount = nil ) or ( FSymbol = nil ) or ( FPosition = nil ) then
  begin
    OnTrailingStopEvent( FTrailingStop, false );
  end else
  begin
    ...
    begin
      ...
      if gEnv.Engine.TradeCore.TrdStops.Find( FPosition ) <> nil then
      begin
        ShowMessageLE(  nil , Format('%s, %s 잔고 ' + #13#10 +  '트레일링 스탑 이미 실행중',
          [ FPosition.Account.Name, FPosition.Symbol.ShortCode])    );
        OnTrailingStopEvent( FTrailingStop, false );
        Exit;
      end;

      //FTrailingStop.Param  := GetParam;
      FTrailingStop.init( FPosition );
      if FTrailingStop.CheckParam( GetParam ) then
      begin
        FTrailingStop.OnResult  := OnTrailingStopEvent;
        FTrailingStop.OnMessage := OnTTrailingStopMessage;
...
// ture : 종목변경   false : 종목변경 X
function TOrderBoard.CheckTrailingStop( aSymbol : TSymbol ): boolean;
begin
  Result := true;

  if not FLiqSet.cbTrailingStop.Checked then
    Exit;

  Result :=  MessageDlgLE( nil,
     ' 종목 변경시 트레일링 스탑이 중지됩니다' + #13+#10 + '계속하시겠습니까?"',
    mtConfirmation, [mbOK, mbCancel]) = 1;

  if Result then FLiqSet.cbTrailingStop.Checked := false;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      stLog :=   Format( 'New Fund 이익 Auto Stop : %s, %s, %s, prc:%s, %d, tick:%d, idx:%d, %s, (%s)',
      [
        aStop.Account.Code, aStop.Symbol.code,
  ...
  begin
    ...
    begin
      ...
      stLog :=   Format( 'New Fund 손실 Auto Stop : %s, %s, %s, prc:%s, %d, tick:%d, idx:%d, %s, (%s)',
      [
        aStop.Account.Code, aStop.Symbol.code,
        ifThenStr(  aStop.Side < 0, '매도', '매수' ),
        aStop.Symbol.PriceToStr( aStop.Price ),
...
procedure TOrderBoard.DoLog(stLog: string);
begin
  if Assigned( FOnLogEvent ) then
    FOnLogEvent( Self, LogTitle,  stLog);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      stLog :=   Format( 'New 이익 Auto Stop : %s, %s, %s, prc:%s, %d, tick:%d, idx:%d, %s, (%s)',
      [
        aStop.Account.Code, aStop.Symbol.code,
        ifThenStr(  aStop.Side < 0, '매도', '매수' ),
        aStop.Symbol.PriceToStr( aStop.Price ),
  ...
  begin
    ...
    begin
      ...
      stLog :=   Format( 'New 손실 Auto Stop : %s, %s, %s, prc:%s, %d, tick:%d, idx:%d, %s, (%s)',
      [
        aStop.Account.Code, aStop.Symbol.code,
        ifThenStr(  aStop.Side < 0, '매도', '매수' ),
        aStop.Symbol.PriceToStr( aStop.Price ),
...
procedure TOrderBoard.OnFill(aOrder: TOrder; bPrf, bLos: boolean; iPrf, iLos,
...
begin
  ...
  begin
    ...
    if cbLiqType.ItemIndex = 0 then
      pcValue := pcMarket
    else
      pcValue := pcLimit;
  end;

  OnFill( aOrder, bPrf, bLos, iPrf, iLos, iTick, pcValue );
...
begin
  ...
  OnFillExec( bPrf, bLos, iPrf, iLos, iTick, pcValue );
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      stLog :=   Format( 'New Fund Stop : %s, %s, %s, prc:%s, %d, tick:%d, idx:%d, %s, (%s)',
      [
    ...
    stLog :=   Format( 'New Stop : %s, %s, %s, prc:%s, %d, tick:%d, idx:%d, %s, (%s)',
    [
...
procedure TOrderBoard.OnTrailingStop(bStart: boolean);
begin
  if FIsFund then
    OnFundTrailingStop( bStart )
  else
    OnAcntTrailingStop( bStart );
end;

procedure TOrderBoard.OnTrailingStopEvent(Sender: TObject; Value: boolean);
var
  aColl : TSingleOrderBoardForm;
  aColl2: TsingleFundOrderBoardForm;
begin
  ...
  begin
    ...
    begin
      aColl2 := ( Collection as TOrderBoards).FOwnerForm as TsingleFundOrderBoardForm;
      if aColl2.cbTrailingStop.Checked then
        aColl2.cbTrailingStop.Checked  := false;

      if Value then
      begin
        aColl2.edtBaseLCTick.Text   := IntToStr(FTrailingStop.Param.BaseTick);
        aColl2.edtPLTick.Text       := IntToStr(FTrailingStop.Param.PLTick);
        aColl2.edtLCTick.Text       := IntToStr(FTrailingStop.Param.LCTick );
      end;

    end else
    begin
      aColl := ( Collection as TOrderBoards).FOwnerForm as TSingleOrderBoardForm;
      if aColl.cbTrailingStop.Checked then
        aColl.cbTrailingStop.Checked  := false;

      if Value then
      begin
        aColl.edtBaseLCTick.Text   := IntToStr(FTrailingStop.Param.BaseTick);
        aColl.edtPLTick.Text       := IntToStr(FTrailingStop.Param.PLTick);
        aColl.edtLCTick.Text       := IntToStr(FTrailingStop.Param.LCTick );
      end;
    end;
  end else
  begin
    if (FLiqSet.cbTrailingStop.Checked)  then
    begin
      FLiqSet.cbTrailingStop.Checked := false;
    end;
    // value = true  설정을 되돌린다.
    if Value then
      with FLiqSet do
      begin
        edtBaseLCTick.Text   := IntToStr(FTrailingStop.Param.BaseTick);
        edtPLTick.Text       := IntToStr(FTrailingStop.Param.PLTick);
        edtLCTick.Text       := IntToStr( FTrailingStop.Param.LCTick );
      end;
  end;
end;

procedure TOrderBoard.OnTTrailingStopMessage(Sender: TObject; iMax,
  iCalc: integer; dStopPrc: double);
var
  aColl : TSingleOrderBoardForm;
  aColl2: TSingleFundOrderBoardForm;
begin
  ...
  begin
    ...
    try
      ...
      begin
        ...
        begin
          aColl2 := ( Collection as TOrderBoards).FOwnerForm as TSingleFundOrderBoardForm;
          aColl2.lbMaxTick.Caption  := IntToStr( iMax );
          aColl2.lbCalcTick.Caption := IntTostr( iCalc );
        end else
        begin
          aColl := ( Collection as TOrderBoards).FOwnerForm as TSingleOrderBoardForm;
      ...
      begin
        FLiqSet.lbMaxTick.Caption  := IntToStr( iMax );
        FLiqSet.lbCalcTick.Caption := IntTostr( iCalc );
      end;
    except
    end;
