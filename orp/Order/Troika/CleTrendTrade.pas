...
begin
  ...
  begin
    ...
    begin
      ...
      DoLog( Format('%s 추세%d %s 주문 가격: %.2f, 수량: %d - %.1f > %d', [ aQuote.Symbol.ShortCode,
      ...
      begin
        ...
        begin
          if FParam.OrdCnt2[ptShort] <= aOrdRes.OrdCnt[ptShort] then
            DoLog( Format('상승 추세2 풋 매도 주문 카운트 Over %d', [ aOrdRes.OrdCnt[ptLong] ]))
          else begin
            NewOrder( iQty, -1, aQuote, aOrdRes, dSum, stInvestor );
            DoLog( Format('상승 추세2 풋 매도 주문 -> 투자자 %.1f = %.1f + %.1f ( 건수 : %d > %d ) ',
              [ dSum, dSum1, dSum2, FQuote.Bids.CntTotal , FQuote.Asks.CntTotal ]));
          end;
        end;
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if FParam.OrdCnt2[ptShort] <= aOrdRes.OrdCnt[ptShort] then
            DoLog( Format('하락 추세2 콜매도 주문 카운트 Over %d', [  aOrdRes.OrdCnt[ptShort] ]))
          else begin
            NewOrder( iQty, -1, aQuote, aOrdRes, dSum, stInvestor );
            DoLog( Format('하락 추세2 콜매도 주문 -> 투자자 %.1f = %.1f + %.1f  ( 건수 : %d < %d ) ',
              [ dSum, dSum1, dSum2, FQuote.Bids.CntTotal , FQuote.Asks.CntTotal ]));
          end;
        end;
      end;
