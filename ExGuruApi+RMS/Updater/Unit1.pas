...
begin
  ...
  lbTitle.Caption := Format('%s %s %s',[ GetSecName , GetAppdiv, lbTitle.Caption]);
...
function TForm1.CheckEmpty( aEdt : TLabeledEdit ) : boolean;
begin
...
begin
  ...
  plDown.Left := plLogin.Left;
  plDown.Top  := plLogin.Top;
...
begin
  try
    ...
    if not CheckEmpty( edt0LeeID ) then Exit;
    if not CheckEmpty( edt0LeePW ) then Exit;
      ...
      case iCode of
        0 :
          begin
            if aParser[1] = '' then
            begin
              stResult.Caption  := '사용기간 에러 , 0 까지 사용가능' ;
              Exit;
            end;

            if aParser[2] = '' then
            begin
              stResult.Caption  := '사용가능한 아이디가 없습니다.' ;
              Exit;
            end;
...
begin
   ...
   try
     ...
     ShowMessage( Format('FTP 접속 에러 업데이트 없이 바로 접속함 (%d)', [ GetLastError]) );
   end;
   ExeMainApp;
...
begin
  ...
  FtpInfo.FtpUser    := 'update_sauri';//FIni.GetString(stSection,'ID');
