...
type
  ...
  public
    ...
    property StrFmt2: string read FStrFmt2;
    property Spec  : TMarketSpec read FSpec;
  ...
  begin
    ...
    begin
			if (tail > 0) then
				tail := tail - 0.01;
			//sTail := Format(FStrFmt, [tail]);
      sTail := FormatFloat(FStrFmt2, tail );
      result:= Format('%d''%s', [body, sTail]);
    end;
  end
  else begin
