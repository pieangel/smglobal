...
begin
  ...
  try
    ...
    gLog.Add( lkError, '','', Format('%s 의 가상계좌 로드 실패', [stFileName]));
  end;
...
begin
  ...
  begin
    ...
    begin
{$IFDEF HANA_STOCK}
      stCode  := Copy( aInvestor.Code, 3, Length( aInvestor.Code ) - 3 + 1 );// + '_1';
{$ELSE}
      stCode  := Copy( aInvestor.Code, 5, Length( aInvestor.Code ) - 5 + 1 );// + '_1';
{$ENDIF}
