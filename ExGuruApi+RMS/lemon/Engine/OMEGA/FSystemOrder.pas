...
uses
  ...
  {
  LogCentral,
  AppUtils,  } CleSymbols, GleTypes, cleOrders,GleConsts,
...
type
  TSystemOrderViewType = (vtTarget, vtLink, vtSignal, vtEvent, vtOrder);
...
begin
  StatusBar.Panels[0].Text := 'TradeStation[' +
...
begin
  ...
  begin
    ...
    // 계좌
    if aTargetItem.Account <> nil then
      Item.Caption := aTargetItem.Account.DisplayName
    else
      Item.Caption := '';
    ...
    // 포지션 & 평균단가
    if aTargetItem.Position <> nil then
    begin
      // 포지션 수량
      if aTargetItem.Position.Volume <> 0 then
        Item.SubItems.Add(IntToSTr(aTargetItem.Position.Volume) )
      else
        Item.SubItems.Add('');
      // 평균단가
      if aTargetItem.Symbol <> nil then
      begin
        Item.SubItems.Add(Format('%.*n', [aTargetItem.Symbol.Spec.Precision,
                                         aTargetItem.Position.AvgPrice])) // 평균단가
      end else
      begin
    ...
    begin
      // 평가손익
      if gUseB then
        Item.SubItems.Add(Format('%.0n',[aTargetItem.Position.EntryOTE]))
      else
        Item.SubItems.Add(Format('%.0n',[aTargetItem.Position.EntryOTE]));
      // 매도주문수량
      if aTargetItem.Position.ActiveSellOrderVolume > 0 then
        Item.SubItems.Add(Format('%d',[aTargetItem.Position.ActiveSellOrderVolume]))
      else
        Item.SubItems.Add('');
      // 매수주문수량
      if aTargetItem.Position.ActiveBuyOrderVolume > 0  then
      Item.SubItems.Add(Format('%d',[aTargetItem.Position.ActiveBuyOrderVolume]))
      else
        Item.SubItems.Add('');
    end else
    begin
...
begin
  ...
  if aLinkItem.Symbol = nil then
    ...
    Item.SubItems.Add(aLinkItem.Symbol.Code);
  Item.SubItems.Add(aLinkItem.Signal.Title);
  Item.SubItems.Add(IntToStr(aLinkItem.Multiplier));
end;
...
begin
  ...
  if Pos('시스템주문', stLog) > 0 then
    ...
    aColor := clNavy
  else if Pos('TradeStation', stLog) > 0 then
    aColor := clTeal
  else
    aColor := clBlack;
