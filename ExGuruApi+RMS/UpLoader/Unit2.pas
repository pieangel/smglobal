...
type
  ...
  public
    ...
    function AddLocalFile(stFile: String; dtFile: TDateTime): TFTPFile; overload;
    function AddLocalFile(stFullFile, stFile: String): TFTPFile; overload;
  ...
  public
    ...
    destructor Destroy; override;
      ...
      // interface
    function Upload(aMode: TFTPMode): Integer; overload; // -1=error, else file transfer count
    function Upload( stDir : string ) : integer; overload;
    ...
    function IsDirExists( stDir : string ) : boolean;
...
// called only in 'CopyDir' and 'SyncDir' mode
//
function TFTPFiles.AddLocalFile(stFullFile, stFile: String): TFTPFile;
begin
  Result := AddTarget(stFullFile, stFile);
  if Result <> nil then
  begin
    Result.FExistsLocally := True;
    //Result.FLocalTime := dtFile;
...
function TFTPConnector.Upload(stDir: string): integer;
var
  i: Integer;
  aFile: TFTPFile;
  stSubDir : string;
begin
  ...
  try
    ...
    begin
      ...
      // 아이디
      if not IsDirExists( stDir ) then
        FFTP.MakeDir( stDir );

      //FFTP.ChangeDir( FRemotePath + '\' + stDir  );
        ...
        begin
          ...
          begin
            ...
            DoLog('Uploading ' + aFile.Name);
              // actual transfer
            FFtp.Put( aFile.Name,
                     ComposeFilePath([FRemotePath + '\' + stDir , aFile.TransferAs]));
            aFile.FTransfered := True;
            Inc(Result);
          end;
        end;
...
function TFTPConnector.IsDirExists(stDir: string): boolean;
var
  aList : TStringList;
  I: Integer;
begin
  Result := false;
  try
    FFtp.List;// .ListParserClass  := TIdFTPLPWindowsNT;
    aList := TStringList.Create;
    for I := 0 to FFtp.DirectoryListing.Count - 1 do
      if FFtp.DirectoryListing.Items[i].ItemType = ditDirectory then
        aList.Add( FFtp.DirectoryListing.Items[i].FileName );
    Result := aList.IndexOf( stDir) >= 0;
  finally
    aList.Free;
  end;
end;
